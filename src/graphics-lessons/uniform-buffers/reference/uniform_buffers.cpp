...
int main()
{
    ...
    unsigned int uboColor;
    glGenBuffers(1, &uboColor);
    glBindBuffer(GL_UNIFORM_BUFFER, uboColor);
    glBufferData(GL_UNIFORM_BUFFER, 1 * sizeof(glm::vec4), NULL, GL_STATIC_DRAW);
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
    glBindBufferRange(GL_UNIFORM_BUFFER, 1, uboColor, 0, sizeof(glm::vec3));
